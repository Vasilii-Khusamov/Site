...
import { arrowLengthPx, cameraHeight, electrostaticDrawDistance, epsilon, gridStepInMeters } from './Consts.js';
...
export function drawIntensityVectorArray(intensityVectorArray, canvasElement, ctx) {
    ...
    for (let i = 0; i < intensityVectorArray.length; i++) {
        ...
        if (intensity > epsilon) {
            ctx.beginPath();
            const direction = multiplyVectorByNumber(normalizeVector(intensityVectorArray[i].intensityVector), arrowLengthPx);
            let color = 0;
            ...
            drawArrow(transformToScreen(new Point(intensityVectorArray[i].position.x * convertToScreen, intensityVectorArray[i].position.y * convertToScreen), canvasElement), transformToScreen(new Point(intensityVectorArray[i].position.x * convertToScreen + direction.x, intensityVectorArray[i].position.y * convertToScreen + direction.y), canvasElement), {
                headArrowWidth: 10,
                headArrowHeight: 10,
