...
import { calculateElectrostaticFieldIntensityVector } from '../../Library/Physics/calculateElectrostaticFieldIntensityVector.js';
import { cameraHeight, epsilon, gridStepInMeters } from './Consts.js';
export function calculateIntensityVectorArray(electricChargeArray) {
    ...
    for (const { x, y } of testPoints) {
        ...
        for (let i = 0; i < electricChargeArray.length; i++) {
            intensityVector = addVectors(intensityVector, calculateElectrostaticFieldIntensityVector(electricChargeArray[i].value, electricChargeArray[i].positionInMeters, new Vector(x, y)));
        }
